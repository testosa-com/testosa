...
const validateTransactionUpdate = (actualRequestObj, skip, hookName) => {
  ...
  const isValidActualRequestObject = ajv.validate(
    transactionActualResponseSchema,
    actualRequestObj
  );
  const isValidSkipValue = typeof skip === 'boolean';

  if (!isValidActualRequestObject) {
    logValidationErrors(ajv.errors, hookName);
  }
    ...
    if (_.isFunction(hook)) {
      ...
      switch (true) {
        case hookName === TEST_HOOK_BEFORE_EACH:
          ...
          nextTransaction.actual.request = isValidTransaction
            ? updatedTransaction.actual.request
            : prevTransaction.actual.request;
          break;

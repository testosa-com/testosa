...
const { getObjectByPath } = require('../utils/data-types');
...
const logFailedResponseBodyAssertion = (errors, data) => {
  ...
  for (let i = 0; i < errors.length; i++) {
    ...
    const trimmedDataPath =
      instancePath === ''
        ? instancePath
        : instancePath.replace('/', '').replaceAll('/', '.');
    ...
    switch (keyword) {
      ...
      case 'type':
        ...
        const receivedValue = getObjectByPath(data, trimmedDataPath);
...
const logTestName = (status, testMetaData) => {
  ...
  const formattedDuration = duration
    ? chalk.hex(LOG_COLOR_RED_SOFT)(`(${duration} ms)`)
    : chalk.hex(LOG_COLOR_RED_SOFT)('---');
  let message = `${statusCode}   ${resolvedPath} ${formattedDuration}`;
